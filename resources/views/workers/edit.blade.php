...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex justify-between items-center mb-10">
            <h1 class="text-3xl font-bold text-gray-900">Edit Worker: {{ $worker->user->name ?? '—' }}</h1>
        ...
        <form method="POST" action="{{ route('workers.update', $worker) }}" class="bg-white rounded-2xl shadow-lg p-10 space-y-10 border border-gray-100">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <input type="tel" name="phone" id="phone" value="{{ old('phone', $worker->user->phone) }}"
                           class="block w-full px-6 py-5 text-lg border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="work_id" id="work_id" value="{{ old('work_id', $worker->work_id) }}"
                           class="block w-full px-6 py-5 text-lg border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                ...
                <div>
                    ...
                    <input type="text" name="nin" id="nin" value="{{ old('nin', $worker->nin) }}"
                           class="block w-full px-6 py-5 text-lg border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                ...
                <div>
                    ...
                    <input type="text" name="department" id="department" value="{{ old('department', $worker->department) }}"
                           class="block w-full px-6 py-5 text-lg border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                ...
                <div>
                    ...
                    <input type="text" name="position" id="position" value="{{ old('position', $worker->position) }}"
                           class="block w-full px-6 py-5 text-lg border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                ...
                <div>
                    ...
                    <input type="date" name="hire_date" id="hire_date" value="{{ old('hire_date', $worker->hire_date?->format('Y-m-d')) }}"
                           class="block w-full px-6 py-5 text-lg border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                ...
                <div>
                    ...
                    <select name="employment_type" id="employment_type"
                            class="block w-full px-6 py-5 text-lg border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                        <option value="permanent" {{ $worker->employment_type === 'permanent' ? 'selected' : '' }}>Permanent</option>
                        <option value="contract" {{ $worker->employment_type === 'contract' ? 'selected' : '' }}>Contract</option>
                        <option value="casual" {{ $worker->employment_type === 'casual' ? 'selected' : '' }}>Casual</option>
                        <option value="probation" {{ $worker->employment_type === 'probation' ? 'selected' : '' }}>Probation</option>
                    </select>
                </div>
            </div>
