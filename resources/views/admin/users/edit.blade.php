...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-lg p-10 space-y-10">
            @csrf
            @method('PUT')

            <!-- Profile Photo -->
            <div class="flex flex-col sm:flex-row items-center gap-10">
                <div class="w-40 h-40 rounded-full overflow-hidden bg-gray-100 flex-shrink-0 border-4 border-gray-200 shadow-inner">
                ...
                <div class="flex-1">
                    <label for="photo" class="block text-xl font-medium text-gray-700 mb-3">Change Profile Photo</label>
                    ...
                    <p class="mt-3 text-sm text-gray-500">JPG, PNG, max 2MB. Leave blank to keep current photo.</p>
                    @error('photo')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div>
                    ...
                    <select name="role" id="role" required 
                        ...
                        <option value="worker" {{ $user->role === 'worker' ? 'selected' : '' }}>Worker / Passenger</option>
                        <option value="admin_level_1" {{ $user->role === 'admin_level_1' ? 'selected' : '' }}>Admin Level 1</option>
                        <option value="admin_level_2" {{ $user->role === 'admin_level_2' ? 'selected' : '' }}>Admin Level 2 (Super Admin)</option>
                    </select>
                    @error('role')
                        <p class="mt-2 text-red-600 text-sm">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div>
                    <label for="password" class="block text-xl font-medium text-gray-700 mb-3">New Password (leave blank to keep current)</label>
                ...
                <div>
                    <label for="password_confirmation" class="block text-xl font-medium text-gray-700 mb-3">Confirm New Password</label>
            ...
            <!-- Submit Buttons -->
            <div class="pt-12 flex justify-end space-x-6">
                <a href="{{ route('admin.users.index') }}" 
                   class="px-12 py-5 bg-gray-200 text-gray-800 font-medium rounded-xl hover:bg-gray-300 transition">
                    Cancel
                </a>
                <button type="submit" 
                        class="px-14 py-5 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 transition shadow-md">
                    Save Changes
                </button>
            </div>
