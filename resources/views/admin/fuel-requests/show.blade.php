...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex justify-between items-start mb-8">
        <div>
            ...
            <p class="mt-2 text-gray-600">
                Requested by: <strong>{{ $fuelRequest->requester->name ?? 'Unknown' }}</strong>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6">
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <dd class="mt-1 text-gray-900 font-medium">{{ number_format($fuelRequest->requested_amount,2) }} L</dd>
                    </div>
                    ...
                    <div>
                        <dt class="text-sm font-medium text-gray-500">Odometer at Request</dt>
                        <dd class="mt-1 text-gray-900">{{ number_format($fuelRequest->odometer_reading) }} km</dd>
                    </div>

                    <div class="col-span-2">
                        <dt class="text-sm font-medium text-gray-500">Reason / Notes</dt>
                        <dd class="mt-1 text-gray-700">{{ $fuelRequest->reason ?? 'No additional notes' }}</dd>
                    </div>

                    <div class="col-span-2">
                        <dt class="text-sm font-medium text-gray-500">Admin Notes</dt>
                        <dd class="mt-1 text-gray-700">{{ $fuelRequest->admin_notes ?? '—' }}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6">
                <h3 class="text-lg font-semibold mb-4">Odometer Photo (Request Time)</h3>
                <img src="{{ $fuelRequest->odometer_photo_url }}" class="max-h-80 rounded-lg shadow-sm object-contain">
            </div>
            @endif

            @if ($fuelRequest->isCompleted() && $fuelRequest->receipt_photo_url)
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6">
                <h3 class="text-lg font-semibold mb-4">Receipt Photo</h3>
                <img src="{{ $fuelRequest->receipt_photo_url }}" class="max-h-96 rounded-lg shadow-sm object-contain">
            </div>
            @endif
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6">
                ...
                <div class="mt-4 pt-4 border-t border-gray-200">
                    <h3 class="text-lg font-semibold mb-2">Payment Request</h3>
                    <p>Amount: <strong>{{ number_format($fuelRequest->paymentRequest->amount,2) }}</strong></p>
                    <p class="mt-1">
                        Status: 
            ...
            <!-- APPROVE / REJECT FOR FUEL REQUEST -->
            @if (in_array($fuelRequest->status,['requested','pending']))

            <!-- APPROVE -->
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6">
                <h3 class="text-lg font-semibold mb-4 text-green-700">Approve Request</h3>
                ...
                <form action="{{ route('admin.fuel-requests.approve',$fuelRequest) }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Approved Litres</label>
                        <input type="number" name="actual_litres_dispensed"
                               ...
                               @if(auth()->id() === $fuelRequest->requested_by) disabled @endif
                               required>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="admin_notes" rows="3"
                                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                                  @if(auth()->id() === $fuelRequest->requested_by) disabled @endif></textarea>
                    </div>
            ...
            <!-- REJECT -->
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6">
                <h3 class="text-lg font-semibold mb-4 text-red-700">Reject Request</h3>
                ...
                <form action="{{ route('admin.fuel-requests.reject',$fuelRequest) }}" method="POST">
                    @csrf
            ...
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6">
                <h3 class="text-lg font-semibold mb-4 text-purple-700">Payment Actions</h3>
                ...
                <!-- Approve Payment -->
                <form action="{{ route('admin.fuel-requests.approvePayment', $fuelRequest) }}" method="POST" class="mb-4">
                    @csrf
                    <button type="submit"
                ...
                <!-- Reject Payment -->
                <form action="{{ route('admin.fuel-requests.rejectPayment', $fuelRequest) }}" method="POST">
                    @csrf
                    <textarea name="notes" rows="3" placeholder="Reason for rejecting payment"
